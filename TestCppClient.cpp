...
int stockAmount;
const Contract& myContract = ContractSamples::ULVR();
//
//while (fin >> stockName >> stockAmount)
//{
...
//    }
	...
	switch (m_state) {
		...
		case ST_BULLETINS_ACK:
			break;
		case ST_ACCOUNTOPERATIONS: // gets position
			accountOperations();
			break;
...
{
    m_pClient->reqPositions(); // https://stackoverflow.com/questions/57618971/how-do-i-get-my-accounts-positions-at-interactive-brokers-using-python-api
    std::cout << "pos:"<< myPos<< std::endl;
	//! [reqpositions]
	std::this_thread::sleep_for(std::chrono::seconds(2));
...
void TestCppClient::nextValidId( OrderId orderId)
{
	...
	//m_state = ST_ACCOUNTOPERATIONS; //has position
...
//		order.lmtPrice, order.auxPrice, orderState.status.c_str(), orderState.completedTime.c_str(), orderState.completedStatus.c_str());
	printf( "Symbol: %s, Action: %s, OrderType: %s, FilledQty: %g, AuxPrice: %g, Status: %s, CompletedStatus: %s\n",
		contract.symbol.c_str(), order.action.c_str(), order.orderType.c_str(), order.filledQuantity,
		order.auxPrice, orderState.status.c_str(), orderState.completedStatus.c_str());
}
//! [completedorder]
